...
function add_DT(){
    ...
    xhtml.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhtml.send(
        `lhvc=${lhvc.value}&sld=${sld.value}&quan=${quan.value}&tp=${tp.value}&namdd=${namdd.value}&name=${name.value}&addr=${addr.value}&phone=${phone.value}&email=${email.value}&username=${username.value}&pass=${password.value}`
    );
}
